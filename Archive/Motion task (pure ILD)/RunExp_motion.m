...
function RunExp_motion(subjectID,block,trials, trialsPr, nconditions, conditions)
...
for trialcount= 1:trialN
    ...
    if trialcount==trialN/2+1
        DrawFormattedText(window,line_rest,'center','center',[1 1 1]); Screen('flip',window);
        waitforbuttonpress;
        Screen('FillRect', window, [0 0 0]); Screen('flip',window);
    end
